...
class GroupShowDos extends React.Component{
  ...
  noEventsMessage (){
    if(!this.props.group.eventnot && !this.props.group.eventboth && !this.props.group.eventeither){
      return (
        <div> No events so far, why not create one? </div>
      );
    }
  }
      ...
      events.map(event => {
        ...
        const randers = (ran=[]) => {
          ...
          for(var i =0; i < ran.length; i++){
            ...
            arr.push(
              <Link to={pj} key={i} className='eventmembertile'> <img className='eventorg' src={ran[i].image_url}></img></Link>);
              }
            return arr;
          };
  ...
  render() {
    ...
    const renderCreateEvent = () => {
        if(this.state.member){
          return(
            <Link className="createeventt2" to={idz +"/create/event"}>Create event</Link>
          );
        }else{
          return(
            <div className='createevent'> Join group to create an event! </div>
          );
        }
      };
        ...
        <div className="singlegroup">
          ...
          <div className="singlegroupmain" style={{backgroundColor: this.props.group.color}}>
            ...
            <div className='memberholder'>
              ...
              <div className='roz'>
                <div className='rmem'>{this.randomGroupMembers()}</div>
